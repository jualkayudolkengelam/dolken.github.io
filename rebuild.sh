...
# Output directory untuk production build
...
clean_build() {
    ...
    if [ -d "$OUTPUT_DIR" ]; then
        rm -rf "$OUTPUT_DIR"/*
        print_info "Cleaned output directory: $OUTPUT_DIR"
    fi
...
build_site() {
    ...
    if bundle exec jekyll build --config "$config_file" --destination "$OUTPUT_DIR" --future; then
