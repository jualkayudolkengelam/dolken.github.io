---
layout: node--product
title: Jual Kayu Dolken Gelam Diameter 5-7 cm
description: Jual kayu dolken gelam diameter 5-7 cm panjang 4 meter. Cocok untuk pagar kokoh, tiang penyangga, dan konstruksi ringan. Harga sudah termasuk ongkir. Bayar setelah barang sampai.
price: 25000
diameter: 5 - 7 cm
min_order: 500
image: "/assets/images/products/jual-kayu-dolken-gelam-5-7cm-001.jpeg"
image_alt: "Kayu dolken gelam diameter 5-7 cm panjang 4 meter untuk pagar kokoh, tiang penyangga, dan konstruksi ringan dengan kekuatan optimal dan harga ekonomis gratis ongkir"
images:
- "/assets/images/products/jual-kayu-dolken-gelam-5-7cm-001.jpeg"
images_alt:
- "Stok kayu dolken gelam diameter 5-7 cm di gudang supplier siap kirim untuk proyek pagar dan konstruksi"
sku: DOLKEN-5-7
popular: false
rating: 4.7
review_count: 5
total_updates: 5
last_modified_at: '2025-12-13 10:00:00 +0700'
show_bulk_offer: false

# Product Description
deskripsi_intro: "Kayu dolken gelam diameter 5-7 cm adalah pilihan populer untuk konstruksi yang membutuhkan kekuatan lebih. Dengan diameter menengah, kayu dolken ini sangat cocok untuk pagar yang kokoh, tiang penyangga pergola, dan berbagai proyek konstruksi ringan hingga sedang."

# Technical Specifications
spesifikasi:
  panjang: "4 meter standar"
  material: "100% kayu gelam asli premium"
  kondisi: "Kering udara, siap pakai"
  kekuatan: "Tinggi, konstruksi ringan-sedang"
  ketahanan: "Maksimal terhadap air, rayap, jamur"
  finishing: "Natural (dapat dicat/divernish)"
  min_order: "500 batang"

# Product Advantages
keunggulan:
- judul: "Keunggulan Utama Produk"
  aplikasi:
  - "Kayu gelam asli kualitas super"
  - "Diameter ideal untuk pagar kokoh"
  - "Cocok untuk tiang penyangga"
  - "Panjang standar 4 meter"
  - "Daya dukung beban menengah"
  - "Harga sudah termasuk ongkir"
  - "Pembayaran setelah barang sampai"
  - "Minimum order 500 batang"
- judul: "Kekuatan Optimal"
  deskripsi: "Diameter 5-7 cm memberikan kekuatan struktural yang baik untuk berbagai aplikasi konstruksi ringan hingga sedang. Keseimbangan sempurna antara kekuatan dan kemudahan handling."
- judul: "Aplikasi Konstruksi"
  aplikasi:
  - "Pagar rumah dan perumahan"
  - "Tiang penyangga pergola"
  - "Konstruksi gazebo"
  - "Pembatas lahan"
  - "Rangka struktur taman"
  - "Tiang utilitas ringan"
  - "Konstruksi semi-permanen"
- judul: "Investasi Jangka Panjang"
  deskripsi: "Dengan ukuran dan kekuatan ini, kayu dolken gelam dapat bertahan puluhan tahun bahkan dalam kondisi outdoor. Sangat ekonomis untuk proyek jangka panjang."

# FAQ Recommendations
faq_rekomendasi:
  header:
    judul: "Rekomendasi Penggunaan"
    icon: "bi-lightbulb-fill"
    tagline: "Tips dan rekomendasi untuk hasil optimal"
  intro:
    judul: "Panduan dan Saran Penggunaan"
    icon: "bi-stars"
    deskripsi: "Rekomendasi dari para profesional untuk memaksimalkan penggunaan produk ini"
  items:
    - pertanyaan: "Bagaimana cara menggunakan kayu dolken diameter 5-7 cm untuk pagar rumah?"
      jawaban: "Untuk pagar rumah, gunakan jarak ideal antar dolken 10-15 cm untuk pagar yang kuat dan rapat. Tinggi standar 1.5-2 meter memberikan privacy optimal dengan tampilan kokoh dan natural."
    - pertanyaan: "Apakah kayu dolken diameter 5-7 cm cocok untuk pergola dan gazebo?"
      jawaban: "Sangat cocok! Dapat digunakan sebagai tiang penyangga dengan jarak 1.5-2 meter sesuai desain struktur. Ideal untuk gazebo ukuran 3x3 hingga 4x4 meter dengan kekuatan yang memadai."
    - pertanyaan: "Berapa minimum order untuk kayu dolken diameter 5-7 cm?"
      jawaban: "Minimum order 500 batang. Jumlah ini cocok untuk proyek pagar perumahan, pembatas lahan, atau kebutuhan konstruksi menengah."
    - pertanyaan: "Apakah kayu dolken diameter 5-7 cm cocok untuk landscape dan taman?"
      jawaban: "Sangat ideal! Cocok sebagai pembatas area taman, penyangga tanaman vertikal, atau struktur dekoratif outdoor dengan daya tahan tinggi terhadap cuaca ekstrem."

# Order Process
cara_pemesanan:
  header:
    judul: "Cara Pemesanan Kayu Dolken Diameter 5-7 cm"
    icon: "bi-cart-check-fill"
    deskripsi: "Hubungi: {{ site.business.phone }} (Telepon / WhatsApp)"
  intro:
    judul: "Langkah Mudah Pemesanan"
    icon: "bi-list-check"
    deskripsi: "Memesan kayu dolken gelam 5-7 cm untuk pagar dan konstruksi Anda sangat praktis. Ikuti 6 langkah mudah berikut untuk mendapatkan material konstruksi berkualitas tinggi dengan layanan profesional"
  sections:
    - judul: "Proses Awal Pemesanan"
      icon: "bi-clipboard-check"
      deskripsi: "Langkah 1-3: Tentukan spesifikasi, hubungi kami, dan dapatkan konsultasi teknis gratis."
      range: "1-3"
    - judul: "Finalisasi & Pengiriman"
      icon: "bi-truck"
      deskripsi: "Langkah 4-6: Verifikasi pesanan, pengiriman aman dengan armada khusus, bayar COD."
      range: "4-6"
  langkah:
    - nomor: 1
      judul: "Tentukan Spesifikasi"
      deskripsi: "Diameter 5-7 cm ideal untuk pagar kokoh, pergola, dan konstruksi ringan-sedang. Min order 500 batang"
      icon: "bi-rulers"
      color: "primary"
    - nomor: 2
      judul: "Hubungi Kami Sekarang"
      deskripsi: "Kontak langsung via telepon atau WhatsApp untuk diskusi kebutuhan proyek Anda"
      icon: "bi-telephone-fill"
      color: "success"
    - nomor: 3
      judul: "Konsultasi Teknis Gratis"
      deskripsi: "Tim ahli konstruksi kami siap membantu kalkulasi kebutuhan dan rekomendasi pemasangan"
      icon: "bi-chat-dots-fill"
      color: "warning"
    - nomor: 4
      judul: "Verifikasi Detail Pesanan"
      deskripsi: "Konfirmasi jumlah batang (min 500), spesifikasi diameter, dan lokasi proyek"
      icon: "bi-clipboard-check"
      color: "info"
    - nomor: 5
      judul: "Pengiriman Khusus Material"
      deskripsi: "Dikirim dengan armada khusus material konstruksi langsung ke lokasi proyek Anda"
      icon: "bi-truck"
      color: "danger"
    - nomor: 6
      judul: "Bayar Setelah Diterima"
      deskripsi: "Pembayaran COD setelah material sampai, cek kualitas dan jumlah sebelum bayar"
      icon: "bi-cash-coin"
      color: "success"
  cta:
    judul: "Konsultasi Proyek Sekarang!"
    icon: "bi-whatsapp"
    deskripsi: "Hubungi <strong>{{ site.business.phone }}</strong> via WhatsApp untuk konsultasi teknis gratis, cek stok, dan penawaran khusus proyek Anda!"
    tombol_text: "WhatsApp Konsultasi"
    tombol_url: "https://wa.me/{{ site.business.whatsapp }}?text=Halo,%20saya%20butuh%20konsultasi%20untuk%20kayu%20dolken%20gelam%20diameter%205-7%20cm%20untuk%20proyek"
---

<section id="product-deskripsi">
{% include products/block--product-deskripsi.html %}
</section>

<section id="product-keunggulan">
{% include products/block--product-keunggulan.html %}
</section>

<section id="product-spesifikasi-teknis">
{% include products/block--product-spesifikasi-teknis.html %}
</section>

<section id="product-harga-pengiriman">
{% include products/block--product-harga-pengiriman.html %}
</section>

<section id="product-cara-pemesanan">
{% include products/block--product-cara-pemesanan.html %}
</section>

<section id="product-area-pengiriman">
{% include products/block--product-area-pengiriman.html %}
</section>

<section id="product-faq-rekomendasi">
{% include products/block--product-faq-rekomendasi.html %}
</section>
